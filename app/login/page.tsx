...
function LoginContent() {
    const { loginWithGoogle, loginWithDiscord, user } = useAuth();
    const { t } = useLanguage();
        ...
        <div className="container mx-auto px-4 py-20 flex justify-center">
            <Card className="w-full max-w-md p-6">
                ...
                <p className="mt-4 text-center text-sm">
                    {t("auth.login.noAccount")} <Link href="/register" className="text-primary hover:underline">{t("auth.login.registerLink")}</Link>
                </p>
            </Card>
        </div>
    );
